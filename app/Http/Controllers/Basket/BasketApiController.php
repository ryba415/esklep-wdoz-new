        ...
        if ($returnData['status']){
            ...
            //if ($basket->itemsCount > 0){
                ...
                if (count($delivery) > 0){
                    ...
                    $orderId = DB::connection('mysql-esklep')->select('SELECT id FROM ecommerce_orders WHERE identity = ?', [$basketHash]);    
                    ...
                    if (count($orderId) > 0){
                        ...
                        setcookie("basket-identifier", "", time() - 3600, "/");
                        unset($_COOKIE['basket-identifier']);
