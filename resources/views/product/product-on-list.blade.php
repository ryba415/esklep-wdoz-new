<div class="product-container flex-[1 1 0] produkt flex flex-col justify-start items-center self-stretch flex-grow relative gap-3 p-5 rounded-[20px] bg-white border border-[#ddd]">
    <div id="favorite-item-icon-{{$product->id}}" onclick="addProductToFavorite(this,{{$product->id}})" class="add-product-to-favorite cursor-pointer heart coursor-pointer flex flex-col justify-center items-center flex-grow-0 flex-shrink-0 h-[40px] w-[40x] absolute right-[12px] top-[12px] gap-2.5 px-[7px] py-1 rounded bg-white">
    ...
    <a href="/{{$product->slug}}" class="w-full flex justify-center flex-grow-0 flex-shrink-0">
        <img src="/uploads/images/product/{{$product->image_name}}" class="flex-grow-0 flex-shrink-0 w-[200px] h-[200px] " />
    </a>
    <a href="/{{$product->slug}}" class="self-stretch flex-grow-0 flex-shrink-0 text-lg font-semibold text-left capitalize text-[#3d3d3d]">{{$product->name}}</a> 
    <p class="flex-grow-0 flex-shrink-0 w-full text-base text-left text-[#595959]">{{$product->brand}}@if(!empty($product->content)), {{$product->content}}@endif</p>
        ...
        <div class="flex justify-start items-center self-stretch flex-grow-0 flex-shrink-0 gap-2.5">
            ...
            <div class="border flex flex-row h-10 rounded-lg overflow-hidden grow-0 shrink-0 basis-auto" >
                <button class="minus px-3 w-10 hover:bg-wdoz-border" onclick="executedecrementQuantity(this)">
                    <svg width="14" height="2" viewBox="0 0 22 2" xmlns="http://www.w3.org/2000/svg" class="stroke-black">
