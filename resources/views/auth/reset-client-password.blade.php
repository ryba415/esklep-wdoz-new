    ...
    <div class="flex flex-col md:flex-row w-full gap-5 items-center justify-center pt-10 pb-8">
        ...
        <h3>Reset hasła</h3>
            ...
            <div id="login-form-sucess" class="hidden text-sm text-[#008641] w-[100%] m-auto bg-wdoz-primary-10 rounded-lg mb-5 border-solid border-wdoz-primary border-2 p-2">
                Na podany adres-email wysłano link do resetu hasła. 
            </div>
            <div class="relative">
                <input type="text" id="reset-password-email" class="block px-2.5 pb-2.5 pt-4 w-full text-sm text-wdoz-text-gray bg-transparent rounded-lg border-1 border-wdoz-input-border appearance-none  focus:outline-none focus:ring-0  peer" placeholder=" " style="border: solid 1px gray;"/>
                <span class="text-red-700 block text-xs pt-1 field-error-info hidden">Błąd</span>
                <label for="reset-password-email" class="pointer-events-none absolute text-sm text-gray-500  duration-300 transform -translate-y-4 scale-75 top-2 z-10 origin-[0] bg-white px-2 peer-focus:px-2 peer-focus:text-[515151]  peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/4 peer-placeholder-shown:top-1/4 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-4 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto start-1">Adres e-mail konta klienta</label>
            </div>
...
document.getElementById('reset-password-button').addEventListener("click", (event) => {
        ...
        showGlobalLoader();
        fetch('/reset-client-password-confirm', {
            method: 'POST',
            headers: {
